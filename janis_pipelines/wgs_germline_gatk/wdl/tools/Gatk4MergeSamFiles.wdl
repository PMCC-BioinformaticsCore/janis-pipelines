...
task Gatk4MergeSamFiles {
  ...
  command <<<
    gatk MergeSamFiles \
      ...
      ~{sep=" " prefix("-I ", bams)} \
      ~{if defined(select_first([outputFilename, "~{if defined(sampleName) then sampleName else "generated"}.merged.bam"])) then ("-O " +  '"' + select_first([outputFilename, "~{if defined(sampleName) then sampleName else "generated"}.merged.bam"]) + '"') else ""} \
    ...
    ln -f `echo '~{select_first([outputFilename, "~{if defined(sampleName) then sampleName else "generated"}.merged.bam"])}' | sed 's/\.[^.]*$//'`.bai `echo '~{select_first([outputFilename, "~{if defined(sampleName) then sampleName else "generated"}.merged.bam"])}' `.bai
  >>>
  runtime {
