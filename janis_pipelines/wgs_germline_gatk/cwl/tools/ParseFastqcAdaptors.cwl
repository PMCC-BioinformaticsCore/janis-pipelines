...
requirements:
  InitialWorkDirRequirement:
    ...
    - entry: "\nimport argparse, json, sys\nfrom typing import Optional, List, Dict,\
        ...
        cli.add_argument(\"--cutadapt_adaptors_lookup\", type=str, required=True,\
        \ help='Specifies a file which contains the list of adapter sequences which\
        \ will\\nbe explicity searched against the library. The file must contain\
        \ sets of named adapters in\\nthe form name[tab]sequence. Lines prefixed with\
        \ a hash will be ignored.')\n\nArray = List\nFile = str\nString = str\nFilename\
        \ = str\nInt = int\nFloat = float\nDouble = float\nBoolean = str\nDirectory\
        \ = str\nStdout = str\nclass PythonTool:\n    File = str\n    Directory =\
        \ str\n\n\ntry:\n    def code_block(fastqc_datafiles: List[File], cutadapt_adaptors_lookup:\
        \ File):\n        \"\"\"\n    \n        :param fastqc_datafiles:\n    \n \
        \       :param cutadapt_adaptors_lookup: Specifies a file which contains the\
        \ list of adapter sequences which will\n            be explicity searched\
        \ against the library. The file must contain sets of named adapters in\n \
        \           the form name[tab]sequence. Lines prefixed with a hash will be\
        \ ignored.\n        :return:\n        \"\"\"\n        import mmap, re, csv\n\
        ...
        \ mmap, and then\n            with open(f) as f, mmap.mmap(f.fileno(), 0,\
        \ access=mmap.ACCESS_READ) as fp:\n                overrepresented_sequences_match\
        \ = re.search(adapt_section_query, fp)\n                if overrepresented_sequences_match\
        \ is None:\n                    raise Exception(\n                       \
        ...
        \            \"\"\"\n    \n            rd = csv.reader(StringIO(tbl), delimiter=\"\
        \\t\", quotechar='\"')\n            ret = list(rd)\n            if len(ret)\
        \ == 0:\n                return ret\n            if skip_headers:\n      \
        \          ret.pop(0)  # discard headers\n            return ret\n    \n \
        \       def get_cutadapt_map():\n            \"\"\"\n            Helper method\
        \ to parse the file 'cutadapt_adaptors_lookup' with\n            format: 'name[tab]sequence'\
        \ into the dictionary: '{ sequence: name }'\n            \"\"\"\n        \
        \    cutadapt_map = {}\n            with open(cutadapt_adaptors_lookup) as\
        \ fp:\n                for row in fp:\n                    st = row.strip()\n\
        \                    if not st or st.startswith(\"#\"):\n                \
        \        continue\n    \n                    # In reality, the format is $name[\\\
        t+]$seqence (more than one tab)\n                    # so we'll just split\
        \ on a tab, and remove all the empty elements.\n                    split\
        \ = [f for f in st.split(\"\\t\") if bool(f) and len(f) > 0]\n    \n     \
        \               # Invalid format for line, so skip it.\n                 \
        \   if len(split) != 2:\n                        print(\n                \
        \            f\"Skipping cutadapt line '{st}' as irregular elements ({len(split)})\"\
        ,\n                            file=stderr,\n                        )\n \
        \                       continue\n    \n                    # reverse the\
        \ order from name[tab]sequence to { sequence: tab }\n                    cutadapt_map[split[1]]\
        \ = split[0]\n            return cutadapt_map\n    \n        # Start doing\
        \ the work\n        adaptor_ids = set()\n        for fastqcfile in fastqc_datafiles:\n\
        \            text = get_overrepresented_text(fastqcfile)\n            adaptor_ids\
        \ = adaptor_ids.union(\n                set(a[0] for a in parse_tsv_table(text,\
        \ skip_headers=True))\n            )\n    \n        adaptor_sequences = []\n\
        \    \n        if adaptor_ids:\n            cutadapt_map = get_cutadapt_map()\n\
        \            for aid in adaptor_ids:\n                if aid in cutadapt_map:\n\
        \                    print(\n                        f\"Identified sequence\
        \ '{aid}' as '{cutadapt_map.get(aid)}' in lookup\",\n                    \
        \    file=stderr,\n                    )\n                    adaptor_sequences.append(aid)\n\
        \    \n                else:\n                    print(\n               \
        \         f\"Couldn't find a corresponding sequence for '{aid}' in lookup\
        \ map\",\n                        file=stderr,\n                    )\n  \
