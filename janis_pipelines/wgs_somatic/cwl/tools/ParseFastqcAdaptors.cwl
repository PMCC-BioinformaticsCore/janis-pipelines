...
requirements:
  InitialWorkDirRequirement:
    ...
    - entry: "\nimport argparse, json\nfrom typing import Optional, List\ncli = argparse.ArgumentParser(\"\
        Argument parser for Janis PythonTool\")\ncli.add_argument(\"--fastqc_datafiles\"\
        , nargs='+', required=True)\ncli.add_argument(\"--cutadapt_adaptors_lookup\"\
        , required=True, help='Specifies a file which contains the list of adapter\
        \ sequences which will\\nbe explicity searched against the library. The file\
        ...
        \n        adapt_section_query = (\n            br\"(?s)>>Overrepresented sequences\\\
        t\\S+\\n(.*?)>>END_MODULE\"\n        )\n        # fastqc_datafile could be\
        \ fairly large, so we'll use mmap, and then\n        with open(f) as f, mmap.mmap(f.fileno(),\
        \ 0, access=mmap.ACCESS_READ) as fp:\n            overrepresented_sequences_match\
        \ = re.search(adapt_section_query, fp)\n            if overrepresented_sequences_match\
        \ is None:\n                raise Exception(\n                    f\"Couldn't\
        \ find query ('{adapt_section_query.decode('utf8')}') in {fastqc_datafiles}\"\
        ...
        : adaptor_sequences}\n\n\nargs = cli.parse_args()\nresult = code_block(fastqc_datafiles=args.fastqc_datafiles,\
        \ cutadapt_adaptors_lookup=args.cutadapt_adaptors_lookup)\n\nprint(json.dumps(result))\n\
        \        "
