...
task BwaMemSamtoolsView {
  input {
    ...
    Array[File] reads
    Array[File]? mates
    String outputFilename = "generated-48d2a1d8-ea17-11e9-8b6a-acde48001122.bam"
    String sampleName
  ...
  command {
    ...
    if [ $(dirname "${reference_dict}") != $(dirname "reference") ]; then mv ${reference_dict} $(dirname ${reference}); fi
     \
      ...
      samtools \
      view \
      ${"-o " + if defined(outputFilename) then outputFilename else "generated-48d2b6e6-ea17-11e9-8b6a-acde48001122.bam"} \
  ...
  output {
    File out = if defined(outputFilename) then outputFilename else "generated-48d2a1d8-ea17-11e9-8b6a-acde48001122.bam"
  }
}