...
cli.add_argument("--fastqc_datafiles", nargs='+', required=True)
cli.add_argument("--cutadapt_adaptors_lookup", required=True, help='Specifies a file which contains the list of adapter sequences which will\nbe explicity searched against the library. The file must contain sets of named adapters in\nthe form name[tab]sequence. Lines prefixed with a hash will be ignored.')
...
EOT
    python ParseFastqcAdaptors-script.py \
      ...
      --cutadapt_adaptors_lookup ~{cutadapt_adaptors_lookup}
  >>>
  runtime {
    docker: "python:3.8.1"
