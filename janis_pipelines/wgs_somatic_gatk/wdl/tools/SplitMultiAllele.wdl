...
task SplitMultiAllele {
  input {
    ...
    File reference_fai
    File reference_dict
    String outputFilename = "generated-8a499706-0fca-11ea-a7d3-acde48001122.norm.vcf"
  }
  command {
    ...
    if [ $(dirname "${reference_dict}") != $(dirname "reference") ]; then mv ${reference_dict} $(dirname ${reference}); fi
     \
      ...
      -r ${reference} \
      | \
      ${"> " + if defined(outputFilename) then outputFilename else "generated-8a499e68-0fca-11ea-a7d3-acde48001122.norm.vcf"} \
  ...
  output {
    File out = if defined(outputFilename) then outputFilename else "generated-8a499706-0fca-11ea-a7d3-acde48001122.norm.vcf"
  }
}