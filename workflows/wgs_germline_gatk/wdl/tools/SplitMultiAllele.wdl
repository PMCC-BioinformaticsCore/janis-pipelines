...
task SplitMultiAllele {
  input {
    ...
    File reference_fai
    File reference_dict
    String outputFilename = "generated-e6b2cab2-e018-11e9-b4b3-a0cec8186c53.norm.vcf"
  }
  command {
    ...
    if [ $(dirname "${reference_dict}") != $(dirname "reference") ]; then mv ${reference_dict} $(dirname ${reference}); fi
     \
      ...
      -r ${reference} \
      | \
      ${"> " + if defined(outputFilename) then outputFilename else "generated-e6b2d002-e018-11e9-b4b3-a0cec8186c53.norm.vcf"} \
  ...
  output {
    File out = if defined(outputFilename) then outputFilename else "generated-e6b2cab2-e018-11e9-b4b3-a0cec8186c53.norm.vcf"
  }
}