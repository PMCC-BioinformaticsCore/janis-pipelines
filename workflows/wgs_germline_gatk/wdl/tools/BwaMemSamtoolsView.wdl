...
task BwaMemSamtoolsView {
  input {
    ...
    Array[File] reads
    Array[File]? mates
    String outputFilename = "generated-e6b1a1fa-e018-11e9-b4b3-a0cec8186c53.bam"
    String sampleName
  ...
  command {
    ...
    if [ $(dirname "${reference_dict}") != $(dirname "reference") ]; then mv ${reference_dict} $(dirname ${reference}); fi
     \
      ...
      samtools \
      view \
      ${"-o " + if defined(outputFilename) then outputFilename else "generated-e6b1ba0a-e018-11e9-b4b3-a0cec8186c53.bam"} \
  ...
  output {
    File out = if defined(outputFilename) then outputFilename else "generated-e6b1a1fa-e018-11e9-b4b3-a0cec8186c53.bam"
  }
}