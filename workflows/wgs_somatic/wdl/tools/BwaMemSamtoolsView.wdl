...
task BwaMemSamtoolsView {
  input {
    ...
    Array[File] reads
    Array[File]? mates
    String outputFilename = "generated-f5abdbf8-e018-11e9-af76-a0cec8186c53.bam"
    String sampleName
  ...
  command {
    ...
    if [ $(dirname "${reference_dict}") != $(dirname "reference") ]; then mv ${reference_dict} $(dirname ${reference}); fi
     \
      ...
      samtools \
      view \
      ${"-o " + if defined(outputFilename) then outputFilename else "generated-f5abf976-e018-11e9-af76-a0cec8186c53.bam"} \
  ...
  output {
    File out = if defined(outputFilename) then outputFilename else "generated-f5abdbf8-e018-11e9-af76-a0cec8186c53.bam"
  }
}