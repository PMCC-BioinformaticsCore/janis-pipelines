...
task SplitMultiAllele {
  input {
    ...
    File reference_fai
    File reference_dict
    String outputFilename = "generated-f5b0e986-e018-11e9-af76-a0cec8186c53.norm.vcf"
  }
  command {
    ...
    if [ $(dirname "${reference_dict}") != $(dirname "reference") ]; then mv ${reference_dict} $(dirname ${reference}); fi
     \
      ...
      -r ${reference} \
      | \
      ${"> " + if defined(outputFilename) then outputFilename else "generated-f5b0f00c-e018-11e9-af76-a0cec8186c53.norm.vcf"} \
  ...
  output {
    File out = if defined(outputFilename) then outputFilename else "generated-f5b0e986-e018-11e9-af76-a0cec8186c53.norm.vcf"
  }
}