...
cwlVersion: v1.0
doc: "FastQC is a program designed to spot potential problems in high througput sequencing\
  \ datasets. It runs a set of analyses on one or more raw sequence files in fastq\
  \ or bam format and produces a report which summarises the results.\nFastQC will\
  \ highlight any areas where this library looks unusual and where you should take\
  \ a closer look. The program is not tied to any specific type of sequencing technique\
  \ and can be used to look at libraries coming from a large number of different experiment\
  \ types (Genomic Sequencing, ChIP-Seq, RNA-Seq, BS-Seq etc etc)."
id: fastqc
inputs:
- id: reads
  inputBinding:
